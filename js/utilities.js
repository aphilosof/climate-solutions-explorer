...
/**
 * Escape text for safe use in HTML attributes (quotes, ampersands, angle brackets)
 */
...
export { sanitizeText, sanitizeUrl, sanitizeAttr };
  ...
  if (items && Array.isArray(items) && items.length > 0) {
    ...
    if (items.length > CONTENT_THRESHOLD) {
      ...
      items.forEach(item => {
        ...
        if (itemUrl && itemUrl !== '#') {
          detailsHtml += `<div style="font-weight: 500;"><a href="${sanitizeAttr(itemUrl)}" target="_blank" rel="noopener noreferrer" class="tooltip-link" style="color: #40916c; text-decoration: none;">${itemTitle}</a></div>`;
        } else {
...
export function showSidePanel(sidePanel, d) {
  ...
  // Analytics: which content gets opened (import kept lazy to avoid cycles)
  import('./analytics.js').then(m => m.trackEvent(`panel:${name}`)).catch(() => {});

  // Store items and node data in global variables
  window._currentPanelItems = items;
  window._currentPanelName = name;
  window._currentNodeData = d; // Store D3 hierarchy node for path traversal
  ...
  items.forEach((item, index) => {
    ...
    if (itemUrl && itemUrl !== '#') {
      contentHtml += `<div class="item-title"><a href="${sanitizeAttr(itemUrl)}" target="_blank" rel="noopener noreferrer" class="side-panel-link">${itemTitle}</a></div>`;
    } else {
...
function populateDropdown(elementId, datasetKey, items, onSelect) {
...
export function populateTypeDropdown(allTypes, onSelect) {
  populateDropdown('typeDropdown', 'type', allTypes, onSelect);
}

export function populateTagDropdown(allTags, onSelect) {
  populateDropdown('tagDropdown', 'tag', allTags, onSelect);
}

export function populateAuthorDropdown(allAuthors, onSelect) {
  populateDropdown('authorDropdown', 'author', allAuthors, onSelect);
}

export function populateLocationDropdown(allLocations, onSelect) {
  populateDropdown('locationDropdown', 'location', allLocations, onSelect);
}
...
// Close all dropdowns (and sync the aria-expanded state of their buttons)
...
export function downloadItemsAsTSV(items, nodeName) {
  ...
  // Build TSV from the actual items
  const fields = ['title', 'url', 'author', 'type', 'tags', 'description', 'date'];
  const header = fields.join('\t');
  const rows = items.map(item => {
    return fields.map(f => {
      let val = item[f] || item[f + '_'] || '';
  ...
  const a = document.createElement('a');
  a.href = url;
  a.download = `${(nodeName || 'items').replace(/[^a-zA-Z0-9_-]/g, '_')}.tsv`;
  a.click();
  URL.revokeObjectURL(url);
}

// Extract all CSS rules from stylesheets (used by SVG and PNG export)
...
export function exportVisualizationAsSVG(filters = {}) {
  ...
  // Add dark background rect as first visual element (after style)
  const bgRect = document.createElementNS('http://www.w3.org/2000/svg', 'rect');
  bgRect.setAttribute('width', '100%');
  bgRect.setAttribute('height', '100%');
  bgRect.setAttribute('fill', '#0a1f1a');
...
export function exportVisualizationAsPNG(filters = {}) {
  ...
  // Clone the SVG to avoid modifying the original (tooltip/sidePanel are outside SVG, no need to hide)
  const clonedSvg = svg.cloneNode(true);
  ...
  // Fill dark background to match site theme
...
export function addFavorite(nodeId, nodeData) {
  ...
  // Create favorite entry (store only essential fields to avoid localStorage quota issues)
  const favorite = {
    id: nodeId,
    name: nodeData.data ? nodeData.data.name : nodeData.name,
...
// Helper function to get node path (accepts D3 hierarchy node)
