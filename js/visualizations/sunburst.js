...
export function renderSunburst(data, showTooltip, hideTooltip) {
  ...
  const svg = d3.select('#visualization')
    .append('svg')
    .attr('role', 'img')
    .attr('aria-label', 'Sunburst: radial rings of climate solution categories. Click a segment to zoom in.')
    .attr('width', width)
    .attr('height', height);
  ...
  const path = g.selectAll('path')
    ...
    .on('mouseover', function(event, d) {
      ...
      // Freeze tooltip and make it clickable (touch devices get an auto-hide)
      tooltipLife.freeze(d);
    })
  ...
  // Depth-1 nodes for curved label placement
  const depth1Nodes = root.children ? root.children.filter(d => d.depth === 1) : [];
  ...
  // Tooltip element: click opens the side panel, hover keeps alive, leave hides
  ...
  const resizeHandler = () => {
    ...
    resizeTimer = setTimeout(() => {
      ...
      // Update stored original coordinates so zoom works after resize
      root.descendants().forEach(d => {
        d.x0_orig = d.x0;
        d.x1_orig = d.x1;
        d.y0_orig = d.y0;
