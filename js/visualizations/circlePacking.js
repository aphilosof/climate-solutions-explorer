...
export function renderCirclePacking(data, showTooltip, hideTooltip) {
  ...
  // Add zoom controls (LEFT edge on desktop, BOTTOM on mobile)
  // Position at the edge of the actual circle pack
  const circlePackLeft = (width - diameter) / 2;

  // Responsive positioning: on small screens, position at bottom center
  ...
  if (isMobile) {
    // Mobile: bottom center, horizontal layout - positioned outside viz area
    zoomControls
      .style('left', '50%')
      .style('bottom', '5px')
    ...
    // Desktop: left side, vertical layout - positioned well outside circle pack
    const controlsLeft = Math.max(5, circlePackLeft - 50);
    zoomControls
      .style('left', `${controlsLeft}px`)
      .style('top', '50%')
  ...
  // Zoom out button (geometric zoom)
  zoomControls.append('button')
    .attr('class', 'zoom-btn')
    .attr('title', 'Zoom out')
    .style('width', `${buttonSize}px`)
    ...
    .on('click', () => {
      // Geometric zoom out (20% decrease)
      zoomScale /= 1.2;
      zoomTo(view);
    });
  ...
  window.addEventListener('resize', () => {
    ...
    resizeTimer = setTimeout(() => {
      ...
      // Update zoom controls position (responsive to screen size)
      const circlePackLeft = (width - diameter) / 2;
      const isMobileResize = width < 768;

      if (isMobileResize) {
        // Mobile: bottom center, horizontal layout with background
        zoomControls
          .style('left', '50%')
          .style('bottom', '5px')
          .style('top', null)
        ...
        // Desktop: left side, vertical layout - positioned outside circle pack
        const controlsLeftResize = Math.max(5, circlePackLeft - 50);
        zoomControls
          .style('left', `${controlsLeftResize}px`)
          .style('top', '50%')
          .style('bottom', null)
          .style('transform', 'translateY(-50%)')
          .style('flex-direction', 'column')
