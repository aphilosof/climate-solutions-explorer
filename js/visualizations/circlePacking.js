...
export function renderCirclePacking(data, showTooltip, hideTooltip) {
  ...
  const svg = d3.select('#visualization')
    .append('svg')
    .attr('role', 'img')
    .attr('aria-label', 'Bubbles: nested circles of climate solution categories. Click a circle to zoom in.')
    .attr('width', width)
    .attr('height', height)
  ...
  */
  ...
  const cleanup = () => {
    ...
    if (momentumAnimationFrame) cancelAnimationFrame(momentumAnimationFrame);
  };
